...
wchar_t *g_pClassName = L"CullMode";       // 窗口类名
wchar_t *g_pWindowName = L"剔除状态示例";   // 窗口标题名
...
HRESULT InitDirect3D(HWND hWnd) {
    ...
    // 创建顶点缓存
    // 创建点集合内存块LPDIRECT3DVERTEXBUFFER9 g_pVertexBuf
    g_pd3dDevice->CreateVertexBuffer(
        6 * sizeof(CUSTOMVERTEX), // Lenth          参数是指这段内存存储多少个Vertex
        0,                        // Usage          可以为0，意味着没有值
        D3DFVF_CUSTOMVERTEX,      // FVF            则指的是VERTEX的格式
        D3DPOOL_DEFAULT,          // Pool           是用来指示内存的管理方式，D3DPOOL 里有各种枚举具体定义和使用方法
        &g_pVertexBuf,            // ppVertexBuffer 就是这个函数传出的地址，就是分配出来的内存的首地址
        NULL);                    // pSharedHandle  参数为保留参数。有了点几何的缓存后，我们就要把数据装入这片内存了
    ...
    pVertices[4] = CUSTOMVERTEX(220.0f, 320.0f, 0.5f, 1.0f, D3DCOLOR_XRGB(255, 255, 0));   // V2
    pVertices[5] = CUSTOMVERTEX(420.0f, 320.0f, 0.5f, 1.0f, D3DCOLOR_XRGB(0, 0, 255));     // V3
    g_pVertexBuf->Unlock();
    return S_OK;
}
