...
wchar_t *g_pClassName  = L"FillMode";       // 窗口类名
wchar_t *g_pWindowName = L"填充状态示例";   // 窗口标题名
...
VOID Direct3DRender() 
{
    ...
    // 设置填充渲染状态
    if (::GetAsyncKeyState('P') & 0x8000)           // 点填充状态
        g_pd3dDevice->SetRenderState(D3DRS_FILLMODE, D3DFILL_POINT);
    if (::GetAsyncKeyState('W') & 0x8000)           // 线框填充状态
        g_pd3dDevice->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);
    if (::GetAsyncKeyState('S') & 0x8000)           // 面填充状态
