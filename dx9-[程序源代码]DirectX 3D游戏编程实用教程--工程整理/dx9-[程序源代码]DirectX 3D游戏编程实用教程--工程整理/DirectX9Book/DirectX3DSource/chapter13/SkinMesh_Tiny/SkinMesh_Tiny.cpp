...
wchar_t *g_pClassName  = L"SkinMesh_Tiny";       // 窗口类名
wchar_t *g_pWindowName = L"骨骼动画示例";   // 窗口标题名
...
HRESULT InitDirect3D(HWND hWnd, HINSTANCE hInstance) 
{
    ...
    g_pCamera->ResetCameraPos(&D3DXVECTOR3(0.0f, 30.0f, -1000.0f));
...
VOID Direct3DRender(HWND hWnd, FLOAT fTimeDelta) 
{
    ...
    {
        fMoveSpeed = 0.0f;
        g_pAnimController->GetAnimationSetByName("Loiter", &pAnimationSet);
        SmoothChangeAnimation(g_pAnimController, pAnimationSet, fCurrTime);
    }
    if (::GetAsyncKeyState(0x32) & 0x8000f)
    {
        fMoveSpeed = 0.0f;
        g_pAnimController->GetAnimationSetByName("Wave", &pAnimationSet);
        SmoothChangeAnimation(g_pAnimController, pAnimationSet, fCurrTime);
    }
    ...
    {
        fMoveSpeed = 1.2f;
        g_pAnimController->GetAnimationSetByName("Jog", &pAnimationSet);
        SmoothChangeAnimation(g_pAnimController, pAnimationSet, fCurrTime);
    }
    ...
    // 设置世界变换矩阵
    D3DXMATRIX matRole, Rx, Ry, Rs, Rt;
