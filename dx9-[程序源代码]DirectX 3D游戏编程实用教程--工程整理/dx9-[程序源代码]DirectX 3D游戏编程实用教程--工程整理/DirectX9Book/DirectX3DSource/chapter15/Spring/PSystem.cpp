...
// Desc: 
//--------------------------------------------------------------------------------------
CSpringPSystem::CSpringPSystem(IDirect3DDevice9 *pd3dDevice) 
    : CPSystem(pd3dDevice)
{
...
CSpringPSystem::~CSpringPSystem() 
{
}

void CSpringPSystem::ResetParticle(PARTICLEATTRIBUTE *pParticleAttr) 
{
    if (pParticleAttr == NULL) return;
    pParticleAttr->IsAlive  = TRUE;
    pParticleAttr->Position = D3DXVECTOR3(0.0f, 0.0f, 0.0f);    // 粒子的初始位置

    D3DXVECTOR3 min = D3DXVECTOR3(-2.0f, 30.0f, -2.0f);
    D3DXVECTOR3 max = D3DXVECTOR3( 2.0f, 50.0f,  2.0f);
    pParticleAttr->Velocity  = GetRandomVector(&min, &max);     // 粒子的初始速度方向
    D3DXVec3Normalize(&pParticleAttr->Velocity, &pParticleAttr->Velocity);
    pParticleAttr->Velocity *= 20.0f;                           // 粒子的初始速度大小

    pParticleAttr->CurrColor = D3DXCOLOR(1.0f, 1.0f, 1.0f, 0.8f);   // 白色
    pParticleAttr->AgeTime   = 0.0f;
    pParticleAttr->LifeTime  = 4.5f;
}

void CSpringPSystem::UpdatePSystem(FLOAT fTimeDelta) 
{
    std::list<PARTICLEATTRIBUTE*>::iterator i;
    for (i = m_listParticles.begin(); i != m_listParticles.end(); i++)
    {
        if (!(*i)->IsAlive) continue;
        (*i)->Position = (*i)->Velocity * fTimeDelta + (*i)->Position;
        (*i)->Position.y += 0.5f * -9.8f * (fTimeDelta * fTimeDelta);   // 粒子在Y方向上的位置
        if ((*i)->Position.y <= 0.0f) (*i)->Position.y = 0.0f;          // 指定最低位置为0

        (*i)->Velocity.y  = (*i)->Velocity.y + -9.8f * fTimeDelta;      // 粒子的速度
        (*i)->AgeTime   += fTimeDelta;
        if ((*i)->AgeTime >= (*i)->LifeTime)
            ResetParticle((*i));        // 重新设置消亡的粒子
    }
}

void CSpringPSystem::BeginRenderPSystem() 
{
...
void CSpringPSystem::EndRenderPSystem() 
{
