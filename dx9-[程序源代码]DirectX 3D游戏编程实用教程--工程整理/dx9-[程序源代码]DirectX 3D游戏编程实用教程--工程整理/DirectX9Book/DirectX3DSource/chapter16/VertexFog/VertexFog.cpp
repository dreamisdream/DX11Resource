...
wchar_t *g_pClassName  = L"VertexFog";      // 窗口类名
wchar_t *g_pWindowName = L"顶点雾化示例";   // 窗口标题名
...
HRESULT InitDirect3D(HWND hWnd, HINSTANCE hInstance) 
{
    ...
    g_pSkybox->LoadSkybox(L"skybox.dds");
    g_pSkybox->InitSkybox(10, 10, 300);

	//g_pd3dDevice->SetRenderState( D3DRS_AMBIENT,  0xFFFFBB55 );
    return S_OK;
}
...
VOID Direct3DRender() 
{
    ...
    if (g_pKeyStateBuffer[DIK_3] & 0x80) 
    {
        g_pd3dDevice->SetRenderState( D3DRS_FOGENABLE, TRUE );
        g_pd3dDevice->SetRenderState( D3DRS_FOGVERTEXMODE, D3DFOG_EXP2);
    }
    if (g_pKeyStateBuffer[DIK_4] & 0x80) 
    {
        g_pd3dDevice->SetRenderState( D3DRS_FOGENABLE, TRUE );
    ...
    if (g_pKeyStateBuffer[DIK_ADD]      & 0x80) fogDensity += 0.0001f;
    if (g_pKeyStateBuffer[DIK_SUBTRACT] & 0x80) fogDensity -= 0.0001f;
    if (fogDensity <= 0.0f) fogDensity = 0.0f;

	g_pd3dDevice->SetRenderState( D3DRS_FOGCOLOR, 0x00CCCCCC );
    g_pd3dDevice->SetRenderState( D3DRS_FOGSTART,   *(DWORD*)&fogStart );   // fogStart=50
    g_pd3dDevice->SetRenderState( D3DRS_FOGEND,     *(DWORD*)&fogEnd );     // fogEnd = 300
    g_pd3dDevice->SetRenderState( D3DRS_FOGDENSITY, *(DWORD*)&fogDensity ); // fogDensity = 0.01f

    // 启用范围雾化
    if (g_pKeyStateBuffer[DIK_V] & 0x80) 
        g_pd3dDevice->SetRenderState( D3DRS_RANGEFOGENABLE, true );
    if (g_pKeyStateBuffer[DIK_B] & 0x80) 
    ...
    // 绘制天空顶
    g_pSkybox->DrawSkybox(&matWorld, FALSE);
